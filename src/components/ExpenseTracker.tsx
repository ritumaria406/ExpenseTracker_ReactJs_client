...
const ExpenseTracker = () => {
    ...
    const addExpense = async ( event : FormEvent<HTMLFormElement> ) => {
        event.preventDefault();
            console.log(expenseDateRef?.current?.value as string);
        const expense = {
            ...
            setDate: expenseDateRef?.current?.value as string
        } as Omit<IItem, 'id'>;
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colSpan={4} className="text-end">
                                ...
                                {totalByPayee("Ramesh") > totalByPayee("Rahul") &&
                                (
                                 `Pay Ramesh`
                                )}
                            ...
                            <td className="font-monospace text-end">
                                {totalByPayee("Rahul") > totalByPayee("Ramesh") &&
                                (
                                 `${(totalByPayee("Rahul") - totalByPayee("Ramesh"))/2}`
                                )}

                                {totalByPayee("Ramesh") > totalByPayee("Rahul") &&
                                (
                                 `${(totalByPayee("Ramesh") - totalByPayee("Rahul"))/2}`
                                )}
                            </td>
                        </tr>
                    </tbody>
